...
export function UserManagementModule() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch('http://10.60.26.80:8000/api/users', {
        headers: {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem('token') || localStorage.getItem('token');
      const response = await fetch('http://10.60.26.80:8000/api/users/import', {
        method: 'POST',
        headers: { 
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
  ...
  const handleDelete = async (id, name) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = sessionStorage.getItem('token') || localStorage.getItem('token');
        const response = await fetch(`http://10.60.26.80:8000/api/users/${id}`, {
          method: 'DELETE',
  ...
  const handleResetPassword = async (id, name) => {
    ...
    if (result.isConfirmed) {
      try {
        const token = sessionStorage.getItem('token') || localStorage.getItem('token');
        const response = await fetch(`http://10.60.26.80:8000/api/users/${id}/reset-password`, {
          method: 'POST',
