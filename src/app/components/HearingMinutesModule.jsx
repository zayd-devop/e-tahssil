...
export default function HearingMinutesModule() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch('http://192.168.11.116:8000/api/hearing-minutes', {
        method: 'GET',
        headers: { 
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem('token') || localStorage.getItem('token');
      const response = await fetch('http://192.168.11.116:8000/api/hearing-minutes/import', {
        method: 'POST',
        headers: { 
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
  ...
  const handlePrintSingle = async (id, fileNumber) => {
    ...
    const url = `http://192.168.11.116:8000/api/hearing-minutes/print/${id}?clerk_name=${encodeURIComponent(clerkName)}`;
  ...
  const handleMergedPrint = async () => {
    ...
    try {
      ...
      const token = sessionStorage.getItem('token') || localStorage.getItem('token');
      const response = await fetch('http://192.168.11.116:8000/api/hearing-minutes/print-merged', {
        method: 'POST',
        headers: { 
          'Authorization': `Bearer ${token}`,
